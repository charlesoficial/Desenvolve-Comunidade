...
export type SourceSixCapturedPostSeed = {
  spaceSlug: string;
  postId: string;
  title: string;
  body: string;
...
const asset = (name: string) => `/source-six-assets/${name}`;
...
const goat: CapturedAuthor = {
  ...
  avatar: "/source-six-assets/Generated20Image20February200220202620-203_09PM.jpeg-c5fdca6c865d.jpg",
  badge: "Hackudo",
  level: "Nível 2",
  joinedAt: "Membro desde 2 de fevereiro de 2026",
};
...
export const sourceSixCapturedPostSeeds: SourceSixCapturedPostSeed[] = [
  {
    spaceSlug: "seu-progresso",
    postId: "1-mes-de-operacao-com-uma-oferta-daqui",
    title: "1 mês de operação com uma oferta daqui!",
    body: "É gratificante ver o que a P6 está fazendo! É uma comunidade sem igual. Esse resultado é de apenas um mês de operação em uma oferta daqui! 👋",
    kind: "image",
    likes: 8,
    author: member,
  ...
  { spaceSlug: "feed-geral", postId: "opsec-seguranca-operacional", title: "🔐 OPSEC: Segurança Operacional", body: "Para os demais conteúdos envolvendo ferramentas, métodos hacking, cripto, biblioteca e muito mais, acesse a Área Hacking.\n\n### Área Hacking\nA aula do dia 10.01.2026 teve como base o assunto de segurança operacional. Neste documento estou relatando todos os pontos importantes para manter uma operação mais segura.", kind: "image", pinned: true, author: goat, attachments: [image("image-85727d05a36e.png", "OPSEC")] },
  { spaceSlug: "feed-geral", postId: "aproveitem-sem-moderacao", title: "APROVEITEM SEM MODERAÇÃO", body: "Ofertas e os links da ads library para vocês, meus rapazes:\n\nhttps://docs.google.com/document/d/1KSrDc6n8SdtHP5YP9a_RnYWTVfDvbBF9aKjt-zPk-Fg/edit?usp=sharing", likes: 3 },
];

export function sourceSixCapturedPostId(seed: Pick<SourceSixCapturedPostSeed, "spaceSlug" | "postId">) {
  return `source-six-${seed.spaceSlug}-${seed.postId}`;
}

export function findSourceSixCapturedPost(spaceSlug: string, postId: string) {
  return sourceSixCapturedPostSeeds.find((post) => (
    post.spaceSlug === spaceSlug
    && (post.postId === postId || sourceSixCapturedPostId(post) === postId)
  ));
}

export function sourceSixCapturedComments(postId: string): FeedComment[] {
  if (!postId.startsWith("source-six-")) return [];
  return [];
}
